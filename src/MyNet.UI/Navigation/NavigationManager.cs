...
namespace MyNet.UI.Navigation
{
    public static class NavigationManager
    {
        ...
        public static void NavigateTo<TPage>(object paramValue, string paramKey = ItemParameter) where TPage : INavigationPage
            => NavigateTo(typeof(TPage), paramValue, paramKey);

        public static bool NavigateTo<TPage>(IEnumerable<KeyValuePair<string, object?>>? parameters = null) where TPage : INavigationPage
        ...
        public static bool NavigateTo(Type typePage, object paramValue, string paramKey = ItemParameter)
            => NavigateTo(typePage, paramValue is null ? null : new List<KeyValuePair<string, object?>>(1)
            {
                new(paramKey, paramValue)
            });
        ...
        public static bool NavigateTo(INavigationPage page, object? paramValue = null, string paramKey = ItemParameter)
            => NavigateTo(page, paramValue is null ? null : new List<KeyValuePair<string, object?>>(1)
            {
                new(paramKey, paramValue)
            });
