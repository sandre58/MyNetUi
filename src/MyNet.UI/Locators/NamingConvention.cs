...
namespace MyNet.UI.Locators
{
    ...
    public static partial class NamingConvention
    {
        ...
        public static string ResolveNamingConvention(Dictionary<string, string> constantsWithValues, string conventionToUse)
            => constantsWithValues.Aggregate(conventionToUse, (current, constantWithValue) => current.Replace(constantWithValue.Key, constantWithValue.Value));
        ...
        public static string ResolveNamingConvention(Dictionary<string, string> constantsWithValues, string conventionToUse, string value)
        {
            ...
            if (lastIndex != -1)
            {
                result = string.Format("{0}{1}", namespaces, conventionToUse.Substring(lastIndex + Up.Length));
            }
        ...
        {
            foreach (var (postfix, lastIndex) in from postfix in postfixesToRemove
                                                 where value.EndsWith(postfix)
                                                 let lastIndex = value.LastIndexOf(postfix, StringComparison.CurrentCultureIgnoreCase)
                                                 select (postfix, lastIndex))
            {
